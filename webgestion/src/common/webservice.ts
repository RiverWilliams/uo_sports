...
class CreneauProvider {
  ...
  public getEnAttentes(id: number): Observable<Inscription[]> {
    ...
    return this.http.get<InscriptionJSON[]>(url).map(value => value.map(Adaptateur.Inscription.fromJSON));
  }
...
class PersonneProvider {
  ...
  public getInscriptions(id: number): Observable<Inscription[]> {
    const url = makeUrl(Urls.PERSONNES_INSCRIPTIONS, {idPersonne: id});
    return this.http.get<InscriptionJSON[]>(url).map(value => value.map(Adaptateur.Inscription.fromJSON));
  }
